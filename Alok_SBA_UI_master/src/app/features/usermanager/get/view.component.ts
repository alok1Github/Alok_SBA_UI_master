...
export class ViewUserComponent implements OnInit {
  ...
  edit(request): void { this.initialiseForm(request); }

  delete(request): void {
    this.userManagerService.deleteUser(request).subscribe(() => { this.getUsers();});
  }

  getUsers(): void { this.userManagerService.getAllUsers().subscribe(this.getResponse()); }

  private getResponse(): (value: User[]) => void {
    return response => {
      this.users = response;
