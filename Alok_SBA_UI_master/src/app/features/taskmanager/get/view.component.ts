...
import { SortOrderType } from 'src/app/shared/enum/sort-order-type';
...
export class ViewComponent implements OnInit {
  ...
  endTask(taskId) {
    ...
    this.service.updateTask(this.taskDetail, this.taskDetail.id).subscribe(
      this.updatesuccessResponse(), this.updateerrorResponse());
  }

  private updateerrorResponse(): (error: any) => void {
    return error => {
      if (error.status < 200 || error.status > 300) {
  ...
  private updatesuccessResponse(): (value: string) => void {
    return response => {
